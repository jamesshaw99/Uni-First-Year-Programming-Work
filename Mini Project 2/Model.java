...
public class Model extends Observable
{
        ...
        synchronized( Model.class )
        {
          ...
          for (int i = 0; i < 11; i++)
          {
              for (int j = 0; j < level+2; j++) {
                  if (j+1 < level)
                    bricks.add(new GameObj(BRICK_WIDTH*(i)+4*i, BRICK_HEIGHT*(j)+4*j+100, BRICK_WIDTH,BRICK_HEIGHT,Colour.RED));
                  else 
                    bricks.add(new GameObj(BRICK_WIDTH*(i)+4*i, BRICK_HEIGHT*(j)+4*j+100, BRICK_WIDTH,BRICK_HEIGHT,Colour.BLUE));
              }
          }
        }
  ...
  protected void looseLife()          { lives--; }
  ...
  public void moveBat( int direction )
  {
    ...
    if(((x >= W - B - BRICK_WIDTH*3) && (direction == 1)) || (( x <= B) && (direction == -1))){
        direction = 0;
    }
      ...
      try
      {
        ...
        while (runGame)
        {
          synchronized ( Model.class ) // Make thread safe
          {
            ...
            { 
              ball.changeDirectionY(); addToScore( HIT_BOTTOM ); looseLife(); 
            }
            ...
            for (GameObj e : bricks) {
                if (e.hitBy(ball) && e.isVisible() && (y > e.getY() || y < e.getY() + BRICK_HEIGHT) && (x < e.getX() || x > e.getX() + BRICK_WIDTH) && !brickHit){
                    ball.changeDirectionY();
                    ball.changeDirectionX();
                }
                else if (e.hitBy(ball) && e.isVisible() && (y > e.getY() || y < e.getY() + BRICK_HEIGHT) && !brickHit){
                    ball.changeDirectionY();
                }
                else if (e.hitBy(ball) && e.isVisible() && (x < e.getX() || x > e.getX() + BRICK_WIDTH) && !brickHit){
                    ball.changeDirectionX();
                }
                
                if(e.hitBy(ball) && e.isVisible() && !brickHit){
                    if(e.getColour() == Colour.RED){
